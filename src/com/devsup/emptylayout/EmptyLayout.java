package com.devsup.emptylayout;
...
import com.devsup.emptylayout.R;
...
public class EmptyLayout {
	...
	public final static int TYPE_CLEAR = 4;

	// ---------------------------
	// default values
	// ---------------------------
	private int mEmptyType = TYPE_LOADING;
	private String mErrorMessage = null;// "Oops! Something wrong happened";
	private String mEmptyMessage = null;// "No items yet";
	private String mLoadingMessage = null;// "Please wait";
	...
	public void setErrorMessage(int errorMessageId) {
		this.mErrorMessage = mContext.getResources().getString(errorMessageId);
	}
	 ...
	 * any item to display
	 * 
	 * @param emptyMessageId
	 *            the message id
	 */
	public void setEmptyMessage(int emptyMessageId) {
		this.mEmptyMessage = mContext.getResources().getString(emptyMessageId);
	}
	 ...
	 * Sets the message to be shown when the list is being loaded
	 * 
	 * @param loadingMessageId
	 *            the message id
	 */
	public void setLoadingMessage(int loadingMessageId) {
		this.mLoadingMessage = mContext.getResources().getString(loadingMessageId);
	}
	...
	private void changeEmptyType() {
			...
			if (!mViewsAdded && mEmptyView.getParent() == null) {
				...
				if (mMainView != null) {
					...
					if (mMainView instanceof ListView) {
						((ListView) mMainView).setEmptyView(rl);
					}
						
				}
			} else if (mEmptyType == TYPE_CLEAR){
				parent.removeAllViews();	
				
				for (int i = 0; i < viewArrayList.size(); ++i) {
					View nextChild = viewArrayList.get(i);
					parent.addView(nextChild);
				}
	...
	private void setDefaultValues() {
		...
		if (mEmptyMessage == null)
			mContext.getResources().getString(R.string.empty_message);

		if (mLoadingMessage == null)
			mContext.getResources().getString(R.string.loading_message);
	 ...
	 */
	public EmptyLayout(Context context, View mainView) {
		mContext = context;
		mInflater = (LayoutInflater) mContext.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
		mMainView = mainView;
	}
